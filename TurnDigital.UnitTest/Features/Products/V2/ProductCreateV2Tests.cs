...
public class ProductCreateV2Tests
{
    ...
    public ProductCreateV2Tests()
    {
        ...
        _validator = new CreateProductV2Validator(readRepository);
    ...
    public async Task Handle_ShouldFailValidation()
    {
        ...
        var handler = new CreateProductV2Handler(_repository, _fileManager.Object, _validator);
        var command = new CreateProductV2(new FileModel(memoryStream, "image.xls", long.MaxValue), null, null, -1, 1);
    ...
    [Fact]
    public async Task Handle_ShouldFailValidation_UniqueNameAndNotFoundCategory()
    {
        // Arrange
        
        var product = await _repository.GetEntitySet<Product>().FirstAsync();
        await using var memoryStream = new MemoryStream();
        var handler = new CreateProductV2Handler(_repository, _fileManager.Object, _validator);
        var command = new CreateProductV2(new FileModel(memoryStream, "image.webp", long.MaxValue), product.Name, null, 500, int.MaxValue);
        ...
        result.Case.Should().BeOfType<FailureResponse<ValidationFailureResponse>>()
            .And.Subject.As<FailureResponse<ValidationFailureResponse>>().ResponseData.As<ValidationFailureResponse>()
            .ValidationMap.Should().ContainKeys("Name", "CategoryId");
    }
    ...
    public async Task Handle_ShouldCreateProduct()
    {
        ...
        await using var memoryStream = new MemoryStream();
        var handler = new CreateProductV2Handler(_repository, _fileManager.Object, _validator);
        var command = new CreateProductV2(new FileModel(memoryStream, "image.webp", long.MaxValue), "Product 1", "Some Description", 500, category.Id);
